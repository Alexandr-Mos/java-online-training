	...
	public void initialise() {
		...
		while (!fileMng.getDirectory().isDirectory()) {
			if (!fileMng.getDirectory().mkdir()) {
				System.out
						.print("Данной директории не существует и её невозможно создать... Введите новый путь:");
		...
		if (!fileMng.existsCatalogFile()) {
			fileMng.setCatalogFile(Catalog.FILE_NAME);
		}
		catalog = new Catalog(fileMng.readCatalogFile());
		
		System.out.println(catalog);

		if (!fileMng.existsUsersFile()) {
			fileMng.setUsersFile(UsersBase.FILE_NAME);
		}
